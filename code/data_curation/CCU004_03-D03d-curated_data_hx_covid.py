...
# MAGIC %md # CCU004_03_D03d_curated_data_hx_covid
# MAGIC
# MAGIC
# MAGIC **Description** This notebook creates the covid phenotypes table of CCU004_03. This notebook creates COVID variables before baseline
# MAGIC
...
# MAGIC
# MAGIC **Data Output**
# MAGIC - **`ccu004_03_cur_hx_covid`** : Covid phenotypes for each person 
...
tmp = tmp\
  .withColumn('priority',substring('covid_phenotype', 1,2).cast(IntegerType()))

#display(tmp)  

win3 = Window\
  .partitionBy(['PERSON_ID'])\
  .orderBy(f.desc('priority'),f.desc('DATE'))   

tmp = tmp\
  .withColumn('_rownum', f.row_number().over(win3))\
  .where(f.col('_rownum') == 1)\
  .drop('priority','_rownum')\
  .withColumnRenamed('DATE', 'exp_hxcovid_date')

count_var(tmp, 'PERSON_ID'); print()
tmp.select('PERSON_ID').distinct().count()

tab(tmp, 'covid_phenotype'); print()
...
tmp.write.partitionBy('source', 'covid_phenotype').mode("overwrite").option('overwriteSchema', 'true').saveAsTable(f'{dsa}.{proj}_cur_hx_covid2_{cohort}')

# COMMAND ----------

tmp = spark.table(f'{dsa}.{proj}_cur_hx_covid2_{cohort}') #cohortrefactoring
...
# tmpt = tabstat(tmp1, 'DATE', byvar='source_pheno', date=1); print() # check crashs - DATE not a column
