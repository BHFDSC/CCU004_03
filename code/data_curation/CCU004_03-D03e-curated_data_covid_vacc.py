...
# MAGIC %md # CCU004_03-D03e-curated_data_covid_vacc
# MAGIC
# MAGIC **Adapted from CCU085_01-D03c-curated_data_covid_vacc**
# MAGIC
# MAGIC **Description** This notebook produces a curated and streamlined version of the vaccine_status table. 
# MAGIC
# MAGIC **Author(s)** Thomas Bolton (Health Data Science Team, BHF Data Science Centre) and adapted by Spencer Keene and Carmen Petitjean
# MAGIC
# MAGIC **Date code last updated** 2024-08-19
# MAGIC
# MAGIC **Data input** 
# MAGIC * `.vaccine_status__archive` (where `archived_on` == YYYY-MM-DD)
# MAGIC * `PROCEDURE.csv`
# MAGIC * `PRODUCT.csv`
# MAGIC
# MAGIC **Data output** 
...
run_level_2_checks = True
run_level_3_checks = True
run_level_x_checks = True
...
import os
print(os.getcwd())
print(f'/Workspace{path_project}/PROCEDURE.csv')
print(f'/Workspace{path_project}/PRODUCT.csv')
print(os.path.exists(f'/Workspace{path_project}/PROCEDURE.csv'))
print(os.path.exists(f'/Workspace{path_project}/PRODUCT.csv'))
...
procedure_csv = pd.read_csv(f'/Workspace{path_project}/PROCEDURE.csv', dtype=str)
product_csv = pd.read_csv(f'/Workspace{path_project}/PRODUCT.csv', dtype=str).fillna('').astype(str)
...
# save
save_table(df=vacc_3, out_name=f'{proj}_cur_covid_vacc_final', save_previous=True)
