...
log using "output\casecohort_discrimination_bysex_c02_`logdate'.log", replace
...
* Store the different sex you have in the dataset as a local variable
levelsof sex, l(interested_sex)
...
foreach modeln in `interested_models' {
	foreach sexi in `interested_sex' {
		foreach impn in `interested_imp' {
			...
			* Prepare the data subset
			keep if (imp == `impn' & score == "`modeln'" & sex == `sexi') 
			
			* Print which subset indicator we are
			display " `modeln':  `sexi': imputation  `impn'  ------------------------------- "
			...
			* Create a temporary dataset with results for the current subgroup
			local sex_subgroup = "sex"
			local subgroup = "sex"	
			local subgroup_cat = "`sexi'"
...
export delimited using "output\c_index_results_c02_bysex_`logdate'.csv", replace
