...
log using "output\casecohort_discrimination_regionsuk_c02_`logdate'.log", replace
...
foreach modeln in `interested_models' {
	foreach reg in `interested_region' {
		foreach impn in `interested_imp' {
			...
			* Prepare the data subset
			keep if (imp == `impn' & score == "`modeln'" & region_uk == "`reg'") 
			
			* Print which subset indicator we are
			display " `modeln':  imputation  `impn' subgroup: `reg'  ------------------------------- "
			...
			* Create a temporary dataset with results for the current subgroup
			...
			local subgroup = "region_uk"	
			local subgroup_cat = "`reg'"
...
export delimited using "output\c_index_results_c02_byregions_`logdate'.csv", replace			
