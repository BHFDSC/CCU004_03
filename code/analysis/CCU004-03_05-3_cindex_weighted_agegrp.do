...
log using "output\casecohort_discrimination_c02_agegrp_`logdate'.log", replace
...
levelsof cindexage, l(interested_agegrp)
...
foreach modeln in `interested_models' {
	foreach impn in `interested_imp' {
		...
		* Restore the dataset
		...
		foreach subgroup_cat in `interested_agegrp' {
			
			* Preserve the datset
			preserve
		
			* Prepare the data subset
			keep if (imp == `impn' & score == "`modeln'" & cindexage == "`subgroup_cat'") 

			
			* Print which subset indicator we are
			display " `modeln':  imputation  `impn' subgroup: `subgroup_cat'  ------------------------------- "
...
export delimited using "output\c_index_results_c02_agegrp_`logdate'.csv", replace	
