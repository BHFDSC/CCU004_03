...
log using "output\casecohort_discrimination_c02_`logdate'.log", replace
...
foreach modeln in `interested_models' {
	foreach impn in `interested_imp' {
		...
		* Print which subset indicator we are
		display " `modeln':  `sex': imputation  `impn'  ------------------------------- "
...
export delimited using "output\c_index_results_c02_`logdate'.csv", replace	
